...
// isPosInt returns true if the input is a string or number that resolves to a positive integer
exports.isInteger = function(arg) {
	if ((c.type(arg) !== 'string') && (c.type(arg) !== 'number')) { return false; }
	if (arg.toString().match(/^\-?[0-9]+$/)) { return true; }
	return false;
};
exports.isPositiveInteger = function(arg) {
	if ((c.type(arg) !== 'string') && (c.type(arg) !== 'number')) { return false; }
	if (arg.toString().match(/^[0-9]+$/)) { return true; }
	return false;
};
...
exports.parseHttpURL = function(arg) {
	...
	if (c.isArrayWithContent(urlParts)) {
...
//	arn:partition:service:region:account-id:resourcetype:resource
	...
	if (c.isArrayWithContent(arnParts)) {
		var arn = {
...
stripObject and stripArray recursively remove key/value pairs or array elements with values that are
		null and undefined, empty objects, or empty arrays.
	c.stripObject(_source);
	c.stripArray(_source);
*/
exports.stripObject = function(_source) {
	var key;
	if (c.isObject(_source)) {
		for (key in _source) {
			if (c.isNothing(_source[key])) { delete _source[key]; }
			else if (c.isObjectWithContent(_source[key])) { c.stripObject(_source[key]); }
			else if (c.isObject(_source[key])) { delete _source[key]; }
			else if (c.isArrayWithContent(_source[key])) { c.stripObject(_source[key]); }
			else if (c.isArray(_source[key])) { delete _source[key]; }
		}
	}
	else if (c.isArray(_source)) {
		for (key = 0; key < _source.length; key++) {
			if (c.isNothing(_source[key])) { _source.splice(key, 1); }
			else if (c.isObjectWithContent(_source[key])) { c.stripObject(_source[key]); }
			else if (c.isObject(_source[key])) { _source.splice(key, 1); }
			else if (c.isArrayWithContent(_source[key])) { c.stripObject(_source[key]); }
			else if (c.isArray(_source[key])) { _source.splice(key, 1); }
		}
	}
	return true;
};
...
checkInput tests input object and returns converted object and errors
	var results = c.checkInput(_data, _fields, _options);
		...
		_fields - array of objects containing field definitions
			[ {
				...
				required	optional, boolean true if the field should exist
				format		optional, one of 'domain', 'email', 'hostname', 'integer' to require that format
	...
	String: regex || values || format
	String actions: strip, lowercase
	String: required || same || different || requiredwith || requiredwithout
	Number: values || min, max || format
	Number: required || same || different || requiredwith || requiredwithout
...
exports.checkInput = function(_data, _fields, _options) {
	...
	var output = { data: {}, errors: [], isModified: false };
	for (var i = 0; i < _fields.length; i++) {
		...
		if (c.isNothing(_data[field.name])) {
			if (field.required) { output.errors.push(prefix + '\'' + field.name + '\' is required'); }
			continue;
		}
		...
		if ((field.type === 'array') && (c.isString(data) || c.isNumber(data) || c.isBoolean(data))) {
			...
			if (c.isNumber(data)) { data = data.toString(); }
			else if (c.isBoolean(data) && data) { data = ''; }
			output.isModified = true;
		} else if (field.type === 'number') {
			if (c.isString(data)) { data = data.parseFloat(); }
		...
		if (hasError) { output.errors.push(prefix + '\'' + field.name + '\' is not a ' + field.type); continue; }
		...
		if (c.isArray(field.values)) {
			...
			if (!found) { output.errors.push(prefix + '\'' + field.name + '\' is not an accepted value'); }
		}
		else if (field.format) {
			var dataArray = data;
			if (!c.isArray(data)) { dataArray = [data]; }
			var fail;
			for (j = 0; j < dataArray.length; j++) {
				var value = dataArray[j];
				if ((field.format === 'domain') && !c.isDomain(value)) { fail = true; }
				if ((field.format === 'hostname') && !c.isHostname(value)) { fail = true; }
				if ((field.format === 'email') && !c.isEmail(value)) { fail = true; }
				if ((field.format === 'httpURL') && !c.isHttpURL(value)) { fail = true; }
				if ((field.format === 'integer') && !c.isPositiveInteger(value)) { fail = true; }
				if ((field.format === 'arn') && !c.isARN(value)) { fail = true; }
				if ((field.format === 'sesARN') && !c.isSesARN(value)) { fail = true; }
			}
			if (fail) { output.errors.push(prefix + '\'' + field.name + '\' is the wrong format'); }
		}
