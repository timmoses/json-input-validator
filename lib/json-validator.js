...
exports.checkInput = function(_data, _fields, _options) {
	...
	var output = { data: {}, error: {}, messages: [], isModified: false };
	var i;
	for (i = 0; i < _fields.length; i++) {
		...
		if (c.isArray(field.values)) {
			...
			if (field.format && !c.isEmpty(data)) {
				var format = field.format.toLowerCase();
				if ((format === 'domain') && !c.isDomain(data)) { fail = true; }
				if ((format === 'hostname') && !c.isHostname(data)) { fail = true; }
				if ((format === 'email') && !c.isEmail(data)) { fail = true; }
				if ((format === 'http url') && !c.isHttpURL(data)) { fail = true; }
				if ((format === 'integer') && !c.isInteger(data)) { fail = true; }
				if ((format === 'positive integer') && !c.isPositiveInteger(data)) { fail = true; }
				if ((format === 'negative integer') && !c.isNegativeInteger(data)) { fail = true; }
				if ((format === 'arn') && !c.isARN(data)) { fail = true; }
				if ((format === 'ses arn') && !c.isSesARN(data)) { fail = true; }
				if (fail) {
					output.messages.push(prefix + '\'' + field.name + '\' is the wrong format');
					output.error[field.name] = 'format';
					continue;
	...
	for (i = 0; i < _fields.length; i++) {
		...
		if (_fields[i].require) {
			...
			var source = output.data[_fields[i].name];
			var target = output.data[parts[2]];
